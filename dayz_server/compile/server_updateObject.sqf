...
// TODO ----------------------
_object_position = {
	private ["_position","_worldspace","_fuel","_key","_ownerPUID"];
	_position = getPosATL _object;
	_ownerPUID = _object getVariable ["ownerPUID","0"];
	//_worldspace = [round (direction _object),_position];
	if (_ownerPUID != "0") then {
		_worldspace = [getDir _object, _position,_ownerPUID] call AN_fnc_formatWorldspace; // Precise Base Building 1.0.5
	} else {
